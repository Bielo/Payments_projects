...
public class HourlyEmployee extends Employee {
    ...
    public BigDecimal calculatePayment(LocalDate date) {
        if (!isPaymentDay(date)) {
            ...
            LocalDate firstDayOfWeek = findFirstDayOfWorkingPeroid(date);
            List<WorkingDay> workingDays = findWorkingDays(firstDayOfWeek, date);
    ...
    private BigDecimal calculatePayment(WorkingDay workingDay) {
        if (workingDay.getHours() <= REGULAR_HOURS) {
            ...
            BigDecimal regularPayment = hourlyRate.multiply(new BigDecimal(REGULAR_HOURS));
            BigDecimal overtimeRate = hourlyRate.multiply(OVERTIME_RATE);
            BigDecimal overtimePayment = overtimeRate.multiply(new BigDecimal(workingDay.getHours() - REGULAR_HOURS));
    ...
    private List<WorkingDay> findWorkingDays(LocalDate firstDay, LocalDate lastDay) {
        return workingDays.stream().filter(workingDay -> workingDay.betweenDays(firstDay, lastDay)).collect(Collectors.toList());
    }
